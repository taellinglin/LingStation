use crate::models::{AudioClipRender, Clip};

use crate::audio::{AudioClipCache, AudioClipData};
use crate::hosts::vst3;
pub use crate::models::{PerformanceClipSettings, PerformanceTriggerMode};
use parking_lot::Mutex;
use std::sync::Arc;

#[derive(Clone, Debug)]
#[allow(dead_code)]
pub struct PerformanceRuntimeClip {
    pub track_index: usize,
    pub launch_samples: u64,
    pub clip: Clip,
...
pub fn performance_clip_loop_beats(runtime: &PerformanceRuntimeClip) -> f32 {
    let clip = &runtime.clip;
    if clip.is_midi {
        clip.midi_source_beats
            .unwrap_or(clip.length_beats)
            .max(0.25)
    } else {
...
pub fn performance_length_samples(runtime: &PerformanceRuntimeClip, samples_per_beat: f64) -> u64 {
...
#[allow(dead_code)]
pub fn collect_performance_block_events(
...
pub fn collect_performance_block_events_into(
    runtime: &PerformanceRuntimeClip,
    block_start: u64,
    block_end: u64,
    samples_per_beat: f64,
    ...
    let recovering_launch_edge =
        block_start > launch_sample && block_start.saturating_sub(launch_sample) < block_len.max(1);
...
pub fn performance_audio_clip_for_block(
    runtime: &PerformanceRuntimeClip,
    _block_start: u64,
    _sample_rate: u32,
    samples_per_beat: f64,
    ...
    let data = audio_cache.try_lock()?.get(&path)?;
    let clip = &runtime.clip;
    ...
    let offset_samples =
        (clip.audio_offset_beats.max(0.0) as f64 * samples_per_beat).round() as u64;
    Some((
        AudioClipRender {
            clip_id: clip.id,
            path: (*path).to_string(),
