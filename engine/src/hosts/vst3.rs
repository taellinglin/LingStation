...
use vst3::Steinberg::Vst::{
    ...
    Event__type0, IAttributeList, IAttributeListTrait, IAttributeList_::AttrID, IAttributeList_iid,
    IAudioProcessor, IAudioProcessorTrait, IComponent, IComponentHandler, IComponentHandlerTrait,
    IComponentHandler_iid, IComponentTrait, IComponent_iid, IConnectionPoint,
    IConnectionPointTrait, IConnectionPoint_iid, IEditController, IEditControllerTrait,
    IEditController_iid, IEventList, IEventListTrait, IHostApplication, IHostApplicationTrait,
    IMessage, IMessageTrait, IMessage_iid, IMidiMapping, IMidiMappingTrait, IParamValueQueue,
    IParamValueQueueTrait, IParameterChanges, IParameterChangesTrait, IPlugInterfaceSupport,
    IPlugInterfaceSupportTrait, IPlugInterfaceSupport_iid, IoModes_, MediaTypes_, NoteOffEvent,
    NoteOnEvent, ParamID, ParamValue, ParameterInfo, ProcessContext, ProcessData, ProcessModes_,
...
use vst3::Steinberg::{
    ...
    IBStream, IBStreamTrait, IPlugFrame, IPlugFrameTrait, IPlugFrame_iid, IPlugView,
    IPlugViewTrait, IPluginBase, IPluginFactory, IPluginFactory3, IPluginFactory3Trait, PClassInfo,
    ViewRect, TUID,
};
...
impl IParamValueQueueTrait for ParamValueQueue {
    ...
    unsafe fn getPoint(
        ...
        if index < 0
            || index as usize >= self.points.len()
            || sample_offset.is_null()
            || value.is_null()
        {
            return kResultFalse;
        }
...
impl HostMessage {
    fn new() -> Self {
        Self {
            ...
            message_id_c: Mutex::new(
                std::ffi::CString::new("").expect("Empty CString creation failed"),
            ),
...
impl IBStreamTrait for MemoryStream {
    ...
    unsafe fn write(
        ...
        if required > self.data.len() {
            ...
            let src = buffer as *const u8;
            let data_ptr = self as *const _ as *mut MemoryStream;
            std::ptr::copy_nonoverlapping(src, (*data_ptr).data.as_mut_ptr().add(cursor), count);
        }
        cursor += count;
        let data_ptr = self as *const _ as *mut MemoryStream;
        ...
        unsafe {
            ...
            if controller_result == kResultOk {
                ...
                if result == kResultOk {
                    if let Some(controller_created) = ComPtr::from_raw(controller_ptr) {
                        ...
                        let count_params = ((*(*controller_created.as_ptr()).vtbl)
                            .getParameterCount)(
            ...
            if let Some(controller) = controller.as_ref() {
                ...
                if let (Some(component_cp), Some(controller_cp)) = (component_cp, controller_cp) {
                    ...
                    if let Some(host_to_controller_ptr) =
                        host_to_controller.to_com_ptr::<IConnectionPoint>()
                    {
                        let _ = component_cp.connect(host_to_controller_ptr.as_ptr());
                        let _ = host_to_controller_ptr.connect(controller_cp.as_ptr());
                    }
                    if let Some(host_to_component_ptr) =
                        host_to_component.to_com_ptr::<IConnectionPoint>()
                    {
...
impl Vst3Editor {
    pub fn attach_hwnd(&mut self, hwnd: isize) -> LingResult<()> {
        ...
        let supported = unsafe {
            self.view
                .isPlatformTypeSupported(type_str.as_ptr() as *const i8)
        };
        log::info!("VST3 isPlatformTypeSupported(HWND) -> {supported}");
        if supported != kResultOk {
            return Err(LingError::Plugin(
                "VST3 view does not support HWND".to_string(),
            ));
        }
...
pub fn enumerate_params(plugin_path: &str) -> LingResult<Vec<ParamInfo>> {
    ...
    unsafe {
        ...
        let get_factory: libloading::Symbol<GetPluginFactoryFn> =
            lib.get(b"GetPluginFactory").map_err(|e| e.to_string())?;
        let factory = get_factory();
        if factory.is_null() {
            return Err(LingError::Plugin(
                "GetPluginFactory returned null".to_string(),
        ...
        for index in 0..count {
            ...
            if controller_result == kResultOk {
                ...
                if result == kResultOk && !controller_ptr.is_null() {
                    if let Some(controller) = ComPtr::from_raw(controller_ptr) {
                        ...
                        let count_params =
                            ((*(*controller.as_ptr()).vtbl).getParameterCount)(controller.as_ptr());
                        for i in 0..count_params {
            ...
            if params.is_empty() {
                ...
                if result == kResultOk && !direct_controller.is_null() {
                    ...
                    let count_params =
                        ((*(*direct_controller).vtbl).getParameterCount)(direct_controller);
                    for i in 0..count_params {
                        let mut info: ParameterInfo = std::mem::zeroed();
...
fn resolve_vst3_binary(plugin_path: &str) -> LingResult<PathBuf> {
    ...
    Err(LingError::Plugin(format!(
        "VST3 binary not found at {plugin_path}"
    )))
}
