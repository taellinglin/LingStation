...
use clack_host::process::audio_buffers::{
    AudioPortBuffer, AudioPortBufferType, AudioPorts, InputAudioBuffers,
};
...
impl ClapHost {
    pub fn load(
        ...
        let host_info =
            HostInfo::new("LingStation", "LingStation", "", "0.1").map_err(|e| e.to_string())?;
        let module_path = resolve_clap_binary(path).map_err(|e| {
            log::error!(
                "CLAP load fail: resolve binary failed path={} err={}",
                path,
                e
            );
            e
        ...
        log::info!(
            "clap_plugin->init begin id={} module={}",
            plugin_id,
            module_path.display()
        );
        ...
        if safe_block_size != block_size {
            eprintln!(
                ...
                block_size, safe_block_size, plugin_id_str
            );
        }
        ...
        log::info!(
            "clap_plugin->activate begin id={} block={}",
            plugin_id_str,
            safe_block_size
        );
        let processor = instance.activate(|_, _| (), audio_config).map_err(|e| {
        ...
        let latency_ext =
            instance.access_shared_handler(|h| h.latency_ext.get().copied().flatten());
    ...
    pub fn open_gui(&mut self, parent_hwnd: isize) -> LingResult<()> {
        let Some(gui) = self.gui_ext else {
            log::error!(
                "CLAP GUI open fail: id={} reason=no_gui_ext",
                self.plugin_id
            );
        ...
        if can_embed {
            log::info!(
                "clap_plugin_gui->create begin id={} mode=embedded",
                self.plugin_id
            );
        ...
        if !gui.is_api_supported(&mut handle, floating) {
            log::error!(
                "CLAP GUI open fail: id={} reason=win32_unsupported",
                self.plugin_id
            );
            return Err(LingError::Plugin(
                "CLAP GUI does not support Win32".to_string(),
            ));
        }
        log::info!(
            "clap_plugin_gui->create begin id={} mode=floating",
            self.plugin_id
        );
    ...
    pub fn take_gui_closed(&mut self) -> bool {
        self.instance
            .access_shared_handler(|h| h.gui_closed.swap(false, Ordering::SeqCst))
    }
        ...
        for event in midi_events {
            match *event {
                VstMidiEvent::NoteOn {
                    ...
                    let pckn = clack_common::events::Pckn::new(0u8, channel, note, 0u8);
                    let event =
                        NoteOnEvent::new(offset, pckn, (velocity as f64 / 127.0).clamp(0.0, 1.0));
                    self.input_events.push(&event);
                }
                VstMidiEvent::NoteOff {
                    ...
                    let pckn = clack_common::events::Pckn::new(0u8, channel, note, 0u8);
                    let event =
                        NoteOffEvent::new(offset, pckn, (velocity as f64 / 127.0).clamp(0.0, 1.0));
                    self.input_events.push(&event);
                }
        ...
        let input_ports = if let Some(input) = input {
            ...
            self.input_ports.with_input_buffers([AudioPortBuffer {
                ...
                channels: AudioPortBufferType::f32_input_only(
                    self.input_buffers
                        .iter_mut()
                        .map(clack_host::process::audio_buffers::InputChannel::variable),
                ),
            }])
        } else {
...
pub fn enumerate_plugins(path: &str) -> LingResult<Vec<ClapPluginDescriptor>> {
    ...
    unsafe {
        ...
        let entry = &*entry_ptr;
        let c_path =
            CString::new(module_path.to_string_lossy().to_string()).map_err(|e| e.to_string())?;
